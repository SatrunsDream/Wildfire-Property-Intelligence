...
\resizebox{0.9\linewidth}{!}{\input{table/modes}}
\end{table}
...
The \texttt{lc\_type} feature, short for land cover type, describes the environment. For this dataset, the landcover types are sourced from \cite{OvertureMaps}. Since there is a wide variety of possible landcover types, it might be useful for some analysis methods to bin similar land cover types together. 
...
As our data set table~\ref{tab:features} is aggregated to cells of H3 level 9, the number of structures per cell (“exposure”) determines the statistical stability of any categorical proportions calculated within that cell. Understanding the distribution of structural exposure across space is essential before testing anomaly detection.
...
\begin{figure}[H]
    ...
    \caption{Color distribution by land cover type for Alpine County.}
...
Let $c$ index counties, $\ell$ index land cover types, and $k$ index colors. We define
\[
n_{c\ell k}
=
...
$w_{c\ell}$ approaches one and the stabilized estimate remains close to the observed proportion. The method trusts large samples and stabilizes small ones. This serves as a sanity check for to reduces false positives caused by sampling variability while preserving meaningful deviations.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.65\linewidth}
        ...
        \caption{County level mean surprisal (averaged over land cover types).}
...
Weighting by $\min(n_{c\ell},n_{c'\ell})$ prioritizes land cover types where both counties have reliable
support.
...
\begin{figure}[H]
    ...
    \caption{County-level maximum incident neighbor Jensen--Shannon divergence (Jensen–Shannon divergence). For each county, the mapped value is the maximum edge score among its adjacent county pairs.}
...
Counties with elevated county divergence scores (defined here as each county's maximum adjacent-edge Jensen–Shannon divergence) exhibit color distributions that differ substantially from adjacent counties within the same landcover types, suggesting potential inconsistencies 
...
where $N_\ell$ is the number of records in land cover $\ell$ across both counties
...
Following the same framing as Jensen–Shannon divergence  $c$ index counties, $\ell$ index land cover types, and $k$ index colors.  
Let $y_{c\ell k}$ denote the number of structures of color $k$ in county $c$
within land cover type $\ell$.
