"""
Export static JSON for /map/neighbor-divergence.

Run from the backend/ directory:
    python export_neighbor_divergence.py

Output: ../../frontend/public/data/neighbor-divergence-map.json
"""
...
OUT_PATH = Path(__file__).parent.parent.parent / "frontend" / "public" / "data" / "neighbor-divergence-map.json"
...
def main():
    ...
    def get_color_distribution(fips_val, lc_type_val):
        ...
        smoothed = np.array(
            [color_counts.get(c, 0) + LAPLACE_PSEUDOCOUNT for c in all_colors], dtype=float
        )
        return smoothed / smoothed.sum()

    print(f"Computing JSD for {len(adjacency_list)} county pairs ...")
    results = []
    ...
    print(f"  {len(edge_features)} edges, {len(county_features)} counties")
